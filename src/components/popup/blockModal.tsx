...
function BlockModal(): JSX.Element {
  const [blockList, setBlockList] = useState<UserData[]>([])

  const { loadUserInfo } = useUser()

  const getBlockList = async (): Promise<UserData[]> => {
    try {
  ...
  useEffect(() => {
    const loadFollowList = async (): Promise<void> => {
      const result: UserData[] = await getBlockList()
      setBlockList(result)
    }
